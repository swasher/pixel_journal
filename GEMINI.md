...
Приложение является **многопользовательским**. Данные каждого пользователя изолированы с помощью `userId`.
...
-   **Коллекция `user_settings`**: Содержит настройки для каждого пользователя.
    -   **Документы:** ID документа **совпадает** с `userId` пользователя.
    -   **Поля документа:**
